...
class GetEdgesShader(StrokeShader):
	...
	def shade(self, stroke):
		...
		for i in range(1,n):
			...
			renderData.freestyle_all_lines.append(s)
		

# Operators.select(AndUP1D(QuantitativeInvisibilityUP1D(0),pyNatureUP1D(Nature.EDGE_MARK)))
# Operators.select(AndUP1D(QuantitativeInvisibilityUP1D(0), OrUP1D(pyNatureUP1D(Nature.SILHOUETTE),pyNatureUP1D(Nature.CREASE),ContourUP1D())))
